...
def monthly_report(orders: pd.DataFrame, customers: pd.DataFrame) -> pd.DataFrame:
    ...
    # orders: every paid order in the group; revenue: the amounts that were recorded
    report = merged.groupby(["region", "month"]).agg(orders=("order_id", "size"), revenue=("amount", "sum")).reset_index()
...
def main() -> None:
    ...
    report.to_csv(OUT, index=False, float_format="%.2f")
